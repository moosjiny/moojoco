...
VORNO_LOG_PATH    = "/home/moos/dev_ws/activity_vorno.jsonl"
...
@app.get("/activity/vorno")
def activity_vorno(limit: int = Query(50, ge=1, le=500)):
    if not os.path.exists(VORNO_LOG_PATH):
        return {"entries": []}
    entries = []
    with open(VORNO_LOG_PATH, "r") as f:
        lines = f.readlines()[-limit:]
    for line in lines:
        line = line.strip()
        if not line:
...
<title>ROOPS 멀티 에이전트 실시간 활동 피드</title>
<style>
...
#log{max-width:980px;}
.row{display:flex;gap:1rem;padding:.4rem 0;border-bottom:1px solid #2a3038;font-size:.85rem;align-items:center;}
.ts{color:#616a74;white-space:nowrap;font-size:.80rem;}
.agent{font-weight:bold;white-space:nowrap;padding:2px 6px;border-radius:4px;font-size:.75rem;}
.agent-moojoco{background:#4c1d0b;color:#ff8a3d;border:1px solid #7c2d12;}
.agent-vorno{background:#064e3b;color:#34d399;border:1px solid #047857;}
.tool{color:#45c7ba;white-space:nowrap;min-width:6em;}
.summary{color:#e8eaeb;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;}
#empty{color:#616a74;}
</style></head><body>
<h1>⚡ ROOPS 멀티 에이전트 실시간 활동 피드 (UTC 기록 → KST+9h 표기)</h1>
<div id="log"><span id="empty">로딩 중...</span></div>
<script>
function toKSTString(tsStr){
  if(!tsStr) return '';
  try {
    let raw = tsStr.trim();
...
async function refresh(){
  try{
    const [resM, resV] = await Promise.all([
      fetch('/activity/moojoco?limit=50').then(r => r.json()).catch(() => ({entries:[]})),
      fetch('/activity/vorno?limit=50').then(r => r.json()).catch(() => ({entries:[]}))
    ]);
    const entriesM = (resM.entries || []).map(e => ({...e, agent: e.agent || 'moojoco'}));
    const entriesV = (resV.entries || []).map(e => ({...e, agent: e.agent || 'vorno'}));
    const all = [...entriesM, ...entriesV].sort((a,b) => (a.ts || '').localeCompare(b.ts || ''));
    ...
    el.innerHTML = all.slice().reverse().map(e => {
      const agentClass = e.agent === 'vorno' ? 'agent-vorno' : 'agent-moojoco';
      const kstFormatted = toKSTString(e.ts);
      return `<div class="row"><span class="ts">${kstFormatted}</span><span class="agent ${agentClass}">[${e.agent}]</span><span class="tool">${e.tool||''}</span><span class="summary">${e.summary||''}</span></div>`;
    }).join('');
  }catch(e){}
}
refresh();
...
FINGER_DOCKING_SCENE_PATH = "/home/moos/dev_ws/dual_arms/data/finger_docking_3d_scene.json"


@app.get("/data/finger-docking-3d-scene.json")
def finger_docking_scene_data():
    with open(FINGER_DOCKING_SCENE_PATH) as fp:
        return JSONResponse(content=json.load(fp))


@app.get("/viz/finger-docking-3d", response_class=HTMLResponse)
def viz_finger_docking_page():
    return HTMLResponse(content=_FINGER_DOCKING_HTML)


@app.get("/data/post-contact-penetration-timeline.png")
def post_contact_penetration_timeline_png():
    return FileResponse(
        "/home/moos/dev_ws/dual_arms/data/post_contact_penetration_timeline.png",
        media_type="image/png",
    )


@app.get("/data/post-contact-penetration-report.json")
def post_contact_penetration_report_json():
    with open("/home/moos/dev_ws/dual_arms/data/post_contact_penetration_report.json") as fp:
        return JSONResponse(content=json.load(fp))
...
_FINGER_DOCKING_HTML = """<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="utf-8">
<title>5손가락 도킹 실측 3D — Moojoco</title>
<style>
html,body{margin:0;height:100%;background:#0f1216;color:#e8eaeb;font-family:ui-monospace,"SF Mono",Consolas,monospace;overflow:hidden;}
#panel{position:fixed;top:0;left:0;z-index:10;padding:1rem 1.2rem;max-width:420px;background:linear-gradient(180deg,rgba(15,18,22,.92),rgba(15,18,22,.55) 80%,transparent);}
#panel h1{font-size:1.0rem;color:#ff8a3d;margin:0 0 .5rem;}
#panel p{font-size:.78rem;line-height:1.5;color:#c3c9d1;margin:.3rem 0;}
#panel a{color:#45c7ba;}
#stats{font-size:.82rem;margin-top:.6rem;padding:.6rem .7rem;border:1px solid #2a3038;border-radius:6px;background:rgba(20,24,30,.7);}
#stats div{display:flex;justify-content:space-between;gap:1rem;padding:1px 0;}
#stats .k{color:#818a94;}
#stats .v{color:#e8eaeb;font-weight:bold;}
#stats .warn{color:#ff5d5d;}
#controls{position:fixed;bottom:0;left:0;right:0;z-index:10;padding:.8rem 1.2rem 1rem;background:linear-gradient(0deg,rgba(15,18,22,.92),transparent);display:flex;align-items:center;gap:.8rem;}
#controls button{background:#1c2128;color:#e8eaeb;border:1px solid #2a3038;border-radius:6px;padding:.4rem .9rem;cursor:pointer;font-family:inherit;font-size:.8rem;}
#controls button:hover{background:#262c35;}
#scrub{flex:1;}
#tlabel{font-size:.78rem;color:#818a94;min-width:9em;text-align:right;}
canvas{display:block;}
#legend{position:fixed;top:1rem;right:1rem;z-index:10;font-size:.75rem;color:#c3c9d1;background:rgba(20,24,30,.7);border:1px solid #2a3038;border-radius:6px;padding:.5rem .7rem;}
#legend div{display:flex;align-items:center;gap:.4rem;padding:2px 0;}
.dot{width:10px;height:10px;border-radius:50%;display:inline-block;}
</style>
</head>
<body>
<div id="panel">
  <h1>5손가락 물리 도킹 — 접촉 유지 실측 3D 재생 (mj_step 실물리)</h1>
  <p>verify_post_contact_penetration.py와 동일한 MuJoCo 시뮬레이션(접근+접촉유지 전 구간 mj_step 500Hz
  실물리, 손목 접근과 손가락 말아쥐기 동기 진행)의 실제 geom xpos/xmat 궤적을 그대로 재생합니다.
  접근(0~4s) 이후 손목 목표를 그대로 3초간 유지(hold)하여 접촉 성립 이후 침투가 발산하는지 확인합니다.
  빨간색으로 표시된 손가락은 해당 프레임에서 <b>data.contact[i].dist &lt; 0</b>(실제 지오메트리 침투)이
  발생한 geom입니다.</p>
  <p>검증 근거: <a href="https://thesis.hyperbook.com/papers/2026-08-05-moojoco-zero-penetration-guideline-empirical-verification" target="_blank">thesis 실측 검증 보고서</a></p>
  <div id="stats">
    <div><span class="k">t</span><span class="v" id="s-t">-</span></div>
    <div><span class="k">phase</span><span class="v" id="s-phase">-</span></div>
    <div><span class="k">접촉쌍 수</span><span class="v" id="s-ncon">-</span></div>
    <div><span class="k">최악 침투(이 프레임)</span><span class="v warn" id="s-worst">-</span></div>
    <div><span class="k">캡슐 반경 대비</span><span class="v warn" id="s-ratio">-</span></div>
  </div>
</div>
<div id="legend">
  <div><span class="dot" style="background:#3bc4f2"></span> Hand A</div>
  <div><span class="dot" style="background:#f2953b"></span> Hand B</div>
  <div><span class="dot" style="background:#ff2d2d"></span> 침투 중(dist&lt;0)</div>
</div>
<div id="controls">
  <button id="play">일시정지</button>
  <input id="scrub" type="range" min="0" max="79" value="0" step="1">
...
rim.position.set(-0.3, 0.3, -0.2);
scene.add(rim);

const grid = new THREE.GridHelper(0.4, 20, 0x2a3038, 0x1c2128);
...
function quatAlignYTo(dir) {
  return new THREE.Quaternion().setFromUnitVectors(Y_AXIS, dir.clone().normalize());
}

let scene_data = null;
let meshes = {};   // name -> {mesh, type, size, baseColor}
let frameIdx = 0;
let playing = true;

async function load() {
  const res = await fetch('/data/finger-docking-3d-scene.json');
  scene_data = await res.json();

  const R = scene_data.capsule_radius_m;
  for (const g of scene_data.geoms_static) {
    let mesh;
    ...
    if (g.type === 'box') {
      const geo = new THREE.BoxGeometry(g.size[0] * 2, g.size[1] * 2, g.size[2] * 2);
      mesh = new THREE.Mesh(geo, mat);
    } else {
      const halfLen = g.size[1];
      const geo = new THREE.CapsuleGeometry(R, halfLen * 2, 6, 12);
      mesh = new THREE.Mesh(geo, mat);
    }
  ...
  scrub.max = scene_data.frames.length - 1;
  applyFrame(0);
  requestAnimationFrame(tick);
}

function applyFrame(idx) {
  const frame = scene_data.frames[idx];
  const penetrating = new Set();
  let worst = 0.0;
  for (const c of frame.contacts) {
    penetrating.add(c.g1);
    penetrating.add(c.g2);
    if (c.d < worst) worst = c.d;
  }
  for (const gdata of frame.geoms) {
    const mesh = meshes[gdata.n];
    if (!mesh) continue;
    mesh.position.set(gdata.p[0], gdata.p[1], gdata.p[2]);
    if (mesh.userData.type === 'box') {
      mesh.quaternion.copy(quatFromMat3(gdata.m));
    } else {
      const dir = new THREE.Vector3(gdata.m[2], gdata.m[5], gdata.m[8]);
      mesh.quaternion.copy(quatAlignYTo(dir));
    }
    const isPen = penetrating.has(gdata.n);
    mesh.material.color.copy(isPen ? new THREE.Color(0xff2d2d) : mesh.userData.baseColor);
    mesh.material.emissive = new THREE.Color(isPen ? 0x550000 : 0x000000);
  }
  document.getElementById('s-t').textContent = frame.t.toFixed(3) + ' s';
  document.getElementById('s-phase').textContent = frame.phase;
  document.getElementById('s-ncon').textContent = frame.contacts.length;
  document.getElementById('s-worst').textContent = (worst * 1000).toFixed(2) + ' mm';
  document.getElementById('s-ratio').textContent = (Math.abs(worst) / scene_data.capsule_radius_m).toFixed(2) + 'x';
  document.getElementById('scrub').value = idx;
  document.getElementById('tlabel').textContent = `frame ${idx} / ${scene_data.frames.length - 1}`;
}
...
function tick(now) {
  requestAnimationFrame(tick);
  controls.update();
  if (playing && scene_data) {
    const dt = now - lastTick;
    if (dt > 1000 / scene_data.fps) {
      lastTick = now;
      frameIdx = (frameIdx + 1) % scene_data.frames.length;
      applyFrame(frameIdx);
    }
  }
