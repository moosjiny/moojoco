...
payload = {
    ...
    "changelog": (
        "v4.0 — §9~11 추가: low_vram_mode+turbo 재시도 2회(diffusers trust_remote_code 문제 해결 후에도 텍스처 "
        "파이프라인 construction 단계에서 재차 OOM, enable_model_cpu_offload가 로딩 단계엔 미적용되는 구조적 한계로 "
        "원인 특정) 및 형상 단독 생성 성공(50.1초, VRAM 5.79GB, 284,444면 메시)을 기록. 최종 결론: 텍스처 포함 "
        "파이프라인은 hb5u 불가, 형상 전용 생성은 실용적 가능 — 파일럿 종료. §0~8은 v3.0 원문 보존."
    ),
    "body_md": BODY_MD,
}
