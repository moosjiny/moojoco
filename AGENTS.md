...
## 통신 채널
- **ntfy 토큰**: `~/.env_roops`의 `NTFY_TOKEN_MOOJOCO` 참조 (하드코딩 금지)
...
- **RHMS**: `https://ec2.hyperbook.com/rhms` (X-Api-Key 헤더, `~/.env_roops`의 `RHMS_KEY_MOOJOCO` 참조)
...
curl -s "https://ntfy.hyperbook.com/roops-moojoco/json?poll=1&since=24h" \
  -H "Authorization: Bearer $NTFY_TOKEN_MOOJOCO"  # source ~/.env_roops 선행
```
...
curl -s "https://ec2.hyperbook.com/rhms/bootstrap?agent=moojoco&hint=session" \
  -H "X-Api-Key: $RHMS_KEY_MOOJOCO"  # source ~/.env_roops 선행
```
...
2. https://thesis.hyperbook.com/papers/2026-06-20-hb5u-Codex-agent-setup
3. https://thesis.hyperbook.com/papers/2026-06-20-Codex-install-nvm
