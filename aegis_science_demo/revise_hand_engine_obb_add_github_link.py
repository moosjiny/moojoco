...
github_url = "https://github.com/moosjiny/moojoco"
commit_url = "https://github.com/moosjiny/moojoco/commit/afdd4ab"
...
**GitHub**: [{github_url}]({github_url}) (커밋: [`afdd4ab`]({commit_url}))
...
## 7. 개정 (v2) — GitHub 링크 추가

사령관 요청으로 문서 상단에 저장소 링크를 추가했다: [{github_url}]({github_url})
(이 보고서가 다루는 변경사항의 커밋: [`afdd4ab`]({commit_url}))
"""

body_md = (body_md
    .replace("{before_url}", before_url)
    .replace("{after_url}", after_url)
...
payload = {
    ...
    "author": "Moojoco",
    "abstract": "v2의 3-캡슐 체인이 원형 단면의 근본 한계로 손바닥 모서리 부근에서 커버리지 구멍(중심 사이 지점 Y-커버리지 부족, 모서리 미검출)을 갖고 있음을 증명. 사령관이 제보한 '손가락이 파고드는' 스크린샷을 정밀 재현해 계산한 결과 해당 프레임 자체는 실제 겹침이 아니라 카메라 원근 착시(모든 간격 0.76-0.99)였음을 밝혔으나, 잠재적 결함을 원천 제거하기 위해 손바닥을 진짜 OBB(오리엔티드 박스)로 교체 — 캡슐-박스는 교대 투영(alternating projection), 박스-박스는 SAT(15축)로 검사. Y축 접근 경계가 이론값과 대수적으로 정확히 일치하고 궤적 슬라이더 전 구간에서 회귀 없이 더 안정적인 결과를 확인. 함께 요청받은 '좌표값 저장' 버튼도 Controls 패널에 추가(Hand A/B 포즈를 JSON으로 저장). [v2 개정] GitHub 저장소 링크 추가.",
    "tags": ["roops", "moojoco", "cpp", "qt6", "collision-detection", "bugfix", "humanoid-hand", "object-oriented-design", "ux"],
    "changelog": "v2.0 — GitHub 저장소 링크 추가 (사령관 요청)",
    "body_md": body_md
}
